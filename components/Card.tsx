...
function Card({ isMobile, blocks, modal, setModal }: iCardProps) {
  ...
  useEffect(() => {
    ...
    if (titleBar != undefined) {
      titleBar.style.background =
        "linear-gradient(90deg,#F0A6CA,#9C89B8)";
    }
    ...
    if (titleBarModal != undefined) {
      titleBarModal.style.background =
        "linear-gradient(90deg,#F0A6CA,#9C89B8)";
    }
  }, []);
  ...
  return (
    <div className="fixed  bg-transparent z-40 top-0 h-full w-full flex justify-center items-center">
      <div
        id="card"
        className={`window transition-all duration-[4000ms] flex ${
          isMobile
            ? "landscape:flex-row portrait:flex-col"
            : "flex-col opacity-0"
        } w-[90vw] static  sm:w-auto sm:px-4  items-center`}
      >
